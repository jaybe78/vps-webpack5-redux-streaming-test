...
import {
  resolveSeoStrategy,
  SeoStrategy,
} from "./renderToStream/resolveSeoStrategy";
...
const globalConfig: { disable: boolean } = ((
  globalThis as any
).__react_streaming = (globalThis as any).__react_streaming || {
  disable: false,
});
function disable() {
...
async function renderToStream(
  ...
  const disable =
    ...
    (options.disable ?? resolveSeoStrategy(options).disableStream);
...
async function renderToNodeStream(
  ...
  const renderToPipeableStream =
    ...
    (await import("react-dom/server")).renderToPipeableStream;
  ...
  const { pipeWrapper, injectToStream, streamEnd } = await createPipeWrapper(
    pipeOriginal,
    {
      onReactBug(err) {
        debug("react bug");
...
async function renderToWebStream(
  ...
  const renderToReadableStream =
    ...
    (await import("react-dom/server")).renderToReadableStream;
  ...
  if (disable) await allReady;
  if (didError) throw firstErr;
  const { readableWrapper, streamEnd, injectToStream } =
    createReadableWrapper(readableOriginal);
  promiseResolved = true;
  return {
