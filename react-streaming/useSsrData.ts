...
function useSsrData<T>(key: string, asyncFn: () => Promise<T>, deps: DependencyList = []): T {
  ...
  console.log(`list of suspenses ${JSON.stringify(suspenses)} on ${isServerSide() ? 'server': 'client'}`)
  ...
  if (suspense.state === 'pending') {
    console.log(`suspense with key:${key} pending on ${isServerSide() ? 'server' : 'client'}`)
    throw suspense.promise
  }
  if (suspense.state === 'error') {
      console.log(`suspense with key:${key} error on ${isServerSide() ? 'server' : 'client'}`)
    throw suspense.error
  }
  if (suspense.state === 'done') {
      console.log(`suspense with key:${key} done on ${isServerSide() ? 'server' : 'client'}`)
